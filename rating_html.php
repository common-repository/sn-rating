...
class RatingHTML {
  ...
  private function _get_rating_box_output($rating) {
    ...
    foreach ($rating['options'] as $rate) { 
      ...
      // rating & aggregation data markup for rich snippets
      $output .= "<span class='rating-single-avg rating' itemscope itemscope itemtype='http://schema.org/Article'>";
      ...
      $output .= "(<span itemprop='ratingValue' class='average'>" . round($rate['aggregate'], 1) . "</span>/";
      $output .= "<span itemprop='bestRating' class='best'>" . $max_count . "</span>)";
      $output .= "<span class='visually-hidden' itemprop='ratingCount'>".$rating['count']."</span></span></span></span>";
      $output .= "</div>";
    }
    if ($rating['multiple'] === true) {
